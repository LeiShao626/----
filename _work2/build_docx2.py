...
p52a = ("从符号指数的视角看，这可以理解为一个受控的转移。在早期阶段（纯 CoT），高 I_S 通过纠错离散化确保准确的词元级梯度。在混合"
        ...
        "I_S，并由渐进式内化稳定下来。")
add_para(p52a, size=10.5, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=6)
p52b = ("在我们的 IL 框架中，这一过程有效地提供了扎根于正确推理的潜思维监督样本，等价于直接从 P_θ*(h|x) 抽样。关键是，分布收敛"
        ...
        "因此，通过课程学习最小化分布散度，可直接优化任务成功率。")
add_para(p52b, size=10.5, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=6)
...
add_para(p61, size=10.5, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=6)
...
add_para(p61b, size=10.5, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=4, indent=18)
p61c = ("• 缺乏离散化（GSM8K）：在 GSM8K 上（图1），模型在没有高概率集中的情况下运行（I_S ≪ 1.0）。根据定理4.11，低 I_S 对应"
        ...
        "导致了更低的计算准确率（34.1%）。")
add_para(p61c, size=10.5, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=6, indent=18)
...
p62c = ("• 显式 CoT（阶跃函数响应）：对低噪声，显式 CoT 的准确率曲线保持平坦，在急剧下降前呈现显著平台。这确认了存在非零的逻辑决策"
        "裕度 Δl。对于噪声向量 ‖ϵ‖ < Δl，离散投影（词元选择）有效地过滤掉扰动（定理4.7）。")
add_para(p62c, size=10.5, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=4, indent=18)
p62d = ("• 潜思维链（连续衰减）：潜思维链表现出立即的单调退化。这确认了在连续潜在表示中 Δl ≈ 0。没有离散裕度，扰动直接改变状态轨迹。"
        "GSM8K 上的衰减速率支持定理4.8，表明转移函数 f_θ 在推理步 M = 6 上累积误差。")
add_para(p62d, size=10.5, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=6, indent=18)
...
p62e = ("图4 显示鲁棒性取决于任务域。虽然 GSM8K 性能快速下降，但 ProsQA 在噪声下相对稳定。这表明探索性任务的解空间是连续的"
        "（h_M 的微小偏差仍映射到有效语义输出），而计算任务要求 h_M 保持在一个狭窄区域内才能产生正确的数值词元。")
add_para(p62e, size=10.5, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=6)
...
p63 = ("我们分析没有课程时潜思维链的训练稳定性（表1：ProntoQA 上 52.4% 准确率）。监测训练过程发现，没有课程，模型在早期 epoch 就收敛"
       ...
       "强制实施低 I_S 推理能力涌现所需的分布对齐是必要的。")
add_para(p63, size=10.5, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=8)
...
p7a = ("在本工作中，我们引入了符号指数，作为一个统一的理论框架，用以解释语言模型推理中探索与执行之间的根本权衡。我们的分析确定"
       ...
       "次优策略中的分布失配，从而确保收敛。")
add_para(p7a, size=10.5, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=6)
p7b = ("虽然符号指数目前是诊断性指标，但它揭示了当前范式设计的一个关键局限：决策确定性的僵化。我们的框架表明，推理架构的进步需要"
       ...
       "探索之间流畅切换的智能体奠定了基础。")
add_para(p7b, size=10.5, align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=8)
